...
Download `luna-ocr-linux-x64` and its checksum from the latest GitHub release, verify it, and place it on `PATH`. Runtime dependencies are Flameshot, `wl-clipboard`, `libnotify-bin`, `systemd-creds`, and optionally ImageMagick. Flameshot runs with `--accept-on-select`, so releasing the mouse immediately submits the region without opening its annotation editor.
